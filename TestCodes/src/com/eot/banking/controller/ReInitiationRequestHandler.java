...
*
* Id: ReInitiationRequestHandler.java,v 1.0
*
* Date Author Changes
* 21 Oct, 2015, 2:57:49 PM Sambit Created
*/
...
/**
 * The Class ReInitiationRequestHandler.
 */
@Controller
@RequestMapping("/reInitiationRequestHandler/*")
public class ReInitiationRequestHandler {
	...
	public void handleRequest(HttpServletRequest request, HttpServletResponse response) {
		...
		int dataLength = headerDataLength == null ? 
				...
				try {
					...
					byte[][] responseData = handler.handleRequest(requestData,true,userName) ;

					response.getOutputStream().write(TLVUtil.getLVArrayFromMultipleData(responseData));
					
				} catch (Exception e1) {
					
					byte[] responseData = messageSource.getMessage("ERROR_"+ErrorConstants.SERVICE_ERROR, null, new Locale("en_US")).getBytes();
